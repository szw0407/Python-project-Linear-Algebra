...
一个简单的 Python 线性代数基础运算的实现

A simple Python implementation of basics in linear algebra.

本项目使用 Conda 管理环境，并使用了 Jupyter Notebook 写参考代码，以便对照需求和讲解。

完整的实现在 `main.py` 中。

## 背景：作为 Python 基础配套练习

这部分作业中目的主要是帮助大家熟悉 Python 的基本语法，以及熟悉 Python 的基本数据结构。

第五步是选做，有兴趣可以了解一下；每一步的难度会逐渐增加，如果你觉得难度太大，可以只做前面的部分。

这次作业需要用到一部分的线性代数相关知识。如果你对线性代数不熟悉，可以先预先学习一下。
...
这个时候我们发现，这个矩阵已经左下角都是0了，没有办法通过类似的方式再化简了。另外，最后一行你发现是 $0=0$，这个方程是多余的，我们忽略就行。然后我们从下往上消元，将第一行加上第二行的$\frac{2}{3}$倍：
...
然后，我们可以将第二行除以 $-3$，以保证每行最左侧第一个不为 0 的元素都是 1：
...
其中，$z$ 是自由变量，这个方程组有无穷多组解。

对于一个有唯一解的方程组，我们可以把这个方程组的增广矩阵化简成这样：
